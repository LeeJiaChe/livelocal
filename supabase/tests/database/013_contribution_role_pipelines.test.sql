...
select lives_ok(
  $$select public.create_spot_draft(
    ...
    'https://images.example.test/mansion.jpg', true, 'image/jpeg', 102400)$$,
  'tourist can submit spot draft');

-- Tourist submits guide
select lives_ok(
  $$select public.submit_guide(
...
select throws_ok(
  $$select public.create_restaurant_draft(
    ...
    'https://images.example.test/kandar.jpg', 'image/jpeg', 102400)$$,
...
select lives_ok(
  $$select public.create_restaurant_draft(
    ...
    'https://images.example.test/hameediyah.jpg', 'image/jpeg', 204800)$$,
...
select lives_ok(
  $$select public.create_restaurant_draft(
    ...
    'https://images.example.test/tohsoon.jpg', 'image/jpeg', 102400)$$,
...
select lives_ok(
  $$select public.admin_moderate_guide_revision(
    ...
    'approved', 'High quality route verified', 1)$$,
...
select lives_ok(
  $$select public.admin_moderate_spot_revision(
    ...
    'approved', 'Verified location and photo', 1)$$,
  'admin approves spot draft');
...
select lives_ok(
  $$select public.admin_moderate_restaurant_revision(
    ...
    'approved', 'Verified video and dishes', 1)$$,
  'admin approves restaurant draft');
...
select throws_ok(
  $$select public.admin_moderate_guide_revision(
    ...
    'approved', 'Unauthorized attempt', 1)$$,
